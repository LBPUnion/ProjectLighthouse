...
public class NewestLevelsCategory : Category
{
    ...
    public override Slot? GetPreviewSlot(Database database) => database.Slots.Where(s => s.Type == "user").OrderByDescending(s => s.FirstUploaded).FirstOrDefault();
    ...
    public override int GetTotalSlots(Database database) => database.Slots.Count(s => s.Type == "user");
}